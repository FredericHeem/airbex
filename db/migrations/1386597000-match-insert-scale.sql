...
BEGIN
    ...
    ELSE
        ask_credit := floor(NEW.price * NEW.volume / 10^(bc_scale));
    END IF;

    RAISE NOTICE 'match_insert: price %, volume %, bid %, ask %', 
        NEW.price,
        NEW.volume,
        bid_credit,
        ask_credit;
    ...
    UPDATE "hold"
    SET
        amount = (1 + asko.fee_ratio) * asko.volume
    ...
    IF asko.fee_ratio > 0 THEN
        ...
        VALUES (
            ...
            ceil(NEW.volume * asko.fee_ratio),
            'MatchFee',
            (SELECT row_to_json(v) FROM (
                    SELECT
                        NEW.match_id
