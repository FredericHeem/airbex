...
BEGIN
    ...
    UPDATE "hold"
    SET
        amount = (1 + asko.fee_ratio) * asko.volume * (10^m.scale)::bigint 
    WHERE hold_id = asko.hold_id;

    -- Reduce bidder hold
    UPDATE "hold"
    SET
        amount = ceil((1 + bido.fee_ratio) * bido.volume * bido.price / (10^(bc_scale - qc_scale))::bigint)
    WHERE
