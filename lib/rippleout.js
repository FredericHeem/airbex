    ...
    return Q.fcall(function() {
        ...
        .fail(function(error) {
            if (error.name == 'malformedTransaction') {
                error.abort = true
                error.name = 'malformed transaction'
            } else if (error.name == 'tecUNFUNDED_PAYMENT') {
                error.abort = true
                error.name = 'unfunded payment'
            } else {
...
RippleOut.prototype.processRequest = function(request) {
    ...
    .then(function(lines) {
        ...
        console.error('[snow:rippleout] Failed to retrieve account lines for %s', request.address)
        console.error('[snow:rippleout]', err)
        err.abort = true
        err.name = 'trust line check failed'
        throw err
    })
}
            ...
            .then(function() {
                ...
                if (error.abort) {
                    ...
                    if (error.retry) {
                        ...
                        return that.abortRequest(request, error.name || 'unknown error')
                    }
                }
