...
var RippleOut = module.exports = function(db, address) {
    ...
    this.drop = new Drop(config.ripple)
...
RippleOut.prototype.executeWithdraw = function(request) {
    ...
    return Q.fcall(function() {
        var amount = {
            ...
            issuer: request.security_id == 'XRP'? null : config.ripple.address
        }

        return that.drop.payment(config.ripple.address, request.address, amount)
...
RippleOut.prototype.processRequest = function(request) {
    ...
    .then(function(lines) {
        ...
        if (!line || diff.lt(0)) {
            if (!line) debug('user has no line to %s', config.ripple.address)
            debug('user is missing %s in his %s line to %s', diff, request.security_id, config.ripple.address)
