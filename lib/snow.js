...
_.extend(Snow.prototype, {
    send: function(method, url, req, cb) {
        ...
        var sec_key_buffer = Buffer(this.options.apiSecret, 'base64');
    ...
    orders: function(cb) {
        ...
        this.send('GET', '/private/orders', function(err, body) {
            if (err) return cb(err);

            cb(null, _.map(body, function(o) {
                var book = _.where(this.books, { book_id: o.book_id })[0];
