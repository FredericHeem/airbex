...
var BitcoinEdge = module.exports = function(options) {
    ...
    this.bitcoin = new Bitcoin(options.endpoint)
...
BitcoinEdge.prototype.configure = function(app) {
    ...
    app.post('/private/deposit/' + self.options.securityId + '/address', function(req, res, next) {
        // todo: does it matter that the user does not own this account?
        if (req.params.account_id) {
            return self.depositAddress(+req.params.account_id, function(err, r) {
                res.send({ address: r })
            })
        }
