...
RippleOut.prototype.getBatch = function() {
    var query = ['SELECT * FROM pop_ripple_withdraw_requests() AS',
            '(request_id int, amount bigint, currency_id currency_id, scale int, address varchar(34));'].join('\n')
    ...
    return Q.fcall(function() {
        ...
        .fail(function(error) {
            if (error.name == 'malformedTransaction') {
                ...
                console.error('unknown error name from drop %s', error.name || '<null>')
            }
...
RippleOut.prototype.loop = function() {
    ...
    .then(function(requests) {
        ...
        return Q.all(requests.map(function(request) {
            ...
            .then(function() {
                console.log('withdraw request %s processed successfully', request.request_id)
            }, function(error) {
