...
RippleIn.prototype.catchupAccount = function(account, fromIndex, toIndex) {
    ...
    .then(function(transactions) {
        ...
        return transactions.length ? Q.all(transactions.map(function(tran) {
            return that.processTransaction(tran)
        })) : null
    })
    .then(function() {
...
// use the securities cache to find the scale used for the security
// can use the num class here to avoid mistakes
RippleIn.prototype.parseAmount = function(amount, securityId) {
    console.log('parsing amount', amount, securityId)
    amount = this.drop.amount(amount).value
    ...
    if (n % 1 !== 0) throw new Error('precision too high')
    return n
}
...
RippleIn.prototype.processTransaction = function(tran) {
    ...
    debug('processing transaction with hash %s', tran.tx.hash)
    assert(tran.tx.hash)
    assert(tran.tx.Amount)

    if (!tran.tx.DestinationTag) {
        debug('ignoring transaction which is missing destination tag')
        return
    }

    var securityId = tran.tx.Currency || 'XRP'
    , amount = this.parseAmount(tran.tx.Amount, securityId)

    // TODO: can update ledger index here

    return this.rippleCredit(tran.tx.hash, securityId, +tran.tx.DestinationTag, amount)
    .fail(function(err) {
...
RippleIn.prototype.onAccountMessage = function(account, message) {
    ...
    this.processTransaction(message.tran)
}
