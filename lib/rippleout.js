...
var RippleOut = module.exports = function(db, address) {
    ...
    dropConfig.secrets[config.RIPPLE_ACCOUNT] = config.RIPPLE_SECRET
...
RippleOut.prototype.executeWithdraw = function(request) {
    ...
    return Q.fcall(function() {
        var amount = {
            value: +num(request.amount, request.scale) + '',
            currency: request.security_id,
            issuer: request.security_id == 'XRP'? null : config.RIPPLE_ACCOUNT
        }

        return that.drop.payment(config.RIPPLE_ACCOUNT, request.address, amount)
...
RippleOut.prototype.processRequest = function(request) {
    ...
    .then(function(lines) {
        ...
        if (!line || diff.lt(0)) {
            if (!line) debug('user has no line to %s', config.RIPPLE_ACCOUNT)
            debug('user is missing %s in his %s line to %s', diff, request.security_id, config.RIPPLE_ACCOUNT)
